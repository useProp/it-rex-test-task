...
describe("map router", () => {
  ...
  describe("add new key-value", () => {
    it("should delete item after ttl expired", async () => {
      ...
      expect(addNewKeyValResponse.body.message).toBe(
        MAP_ROUTER_MESSAGES.SUCCESS,
      );
    ...
    it("should not add new key without key or value", async () => {
      const addNewKeyValResponse = await request(app).post("/map/add");
  ...
  describe("get value by the key", () => {
    it("should return a value by the provided key", async () => {
      map.add(key, value);
      const getValueResponse = await request(app).get(`/map/get/${key}`);
    ...
    it("should return an empty value", async () => {
      const getValueResponse = await request(app).get(`/map/get/${key}`);
      expect(getValueResponse.status).toBe(200);
      expect(getValueResponse.body.item).toBeNull();
    });
  });
  ...
  describe("delete value by the key", () => {
    it("should delete value by the provided key", async () => {
      map.add(key, value);
      const deleteValueResponse = await request(app).delete(
        `/map/remove/${key}`,
      );
      expect(deleteValueResponse.status).toBe(200);
      expect(deleteValueResponse.body.message).toBe(
